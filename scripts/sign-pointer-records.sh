...
#
# The key is read from ~/.config/atlas/root.key — Atlas's ROOT identity, the
# same key that signs the web-container metadata and the KeyAuth authorising the
# online key. It is piped straight into the signer and never lands in a
# variable, a temp file, or argv.
#
# It is a RAW 32-byte ed25519 seed, not a TOML file like River's, so it is
# hex-encoded on the way in. pointer-record accepts hex, base58 or a key file;
# raw binary is the one thing it cannot read, because it reads stdin as text.
set -euo pipefail

REPO_ROOT="$(cd "$(dirname "$0")/.." && pwd)"
TOML_PATH="pointer-records.toml"
KEY_FILE="${ATLAS_POINTER_KEY_FILE:-$HOME/.config/atlas/root.key}"
KEY_IS_RAW_SEED=1
...
# whole trust anchor for every integrator resolving an Atlas pointer. Losing it
# also makes the live Atlas permanently un-updatable (PUBLISHING-KEYS.md).
#
...
# The seed is binary; pointer-record reads stdin as text. Hex it on the way in,
# through a process substitution so it never becomes a shell variable or a file.
hex_key() {
    if [ "${KEY_IS_RAW_SEED:-0}" -eq 1 ]; then xxd -p -c 64 "$KEY_FILE"; else cat "$KEY_FILE"; fi
}
...
for i in $(seq 1 "$N"); do
    ...
    OUT="$(pointer-record sign \
        ...
        --app-id "$APP_ID" \
        --version "$NEW_VERSION" \
        --code-hash "$NEW_HASH" < <(hex_key))" || die "signing $APP_ID failed"
