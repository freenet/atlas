#!/bin/bash
# Publish Atlas's signed pointer records to the Freenet network.
#
...
# Atlas's index params: root_vk (32 bytes) || slug. "default" is the main index.
INDEX_SLUG="${ATLAS_INDEX_SLUG:-default}"
...
RUNS="$(gh run list --repo freenet/atlas --commit "$HEAD_SHA" --json conclusion,status,name 2>/dev/null || echo "")"
...
if [ "$BAD" != "0" ]; then
    gh run list --repo freenet/atlas --commit "$HEAD_SHA" --limit 20 | sed 's/^/    /'
...
for i in $(seq 1 "$N"); do
    ...
    # The committed wasm's own hash must be what the record names.
    COMMITTED_HASH="$(b3sum "$WASM_PATH" | cut -d' ' -f1)"
    [ "$COMMITTED_HASH" = "$CODE_HASH" ] || die "the record names $CODE_HASH
but $WASM_PATH hashes to $COMMITTED_HASH. Re-sign before publishing."

    # ...and the network must actually hold the contract at the address that
    # hash implies. Otherwise the record would advertise a generation nobody is
    # running.
    printf '%s' "$AUTHOR_VK" | xxd -r -p > "$WORK/idxp_$i.bin"
    printf '%s' "$INDEX_SLUG" >> "$WORK/idxp_$i.bin"
...
The committed $WASM_PATH is not the generation that is live. Publishing this
record would point every integrator at code nobody is running. Publish the
contract first, or re-sign against the generation that IS live."
    else
        tail -3 "$WORK/live_$i.log" | sed 's/^/      /'
        die "[2] could not determine whether $TARGET_ID is on the network, and the
node did not say 'not found'. That is 'we learned nothing'. Retry."
    fi
